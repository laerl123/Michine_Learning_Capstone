        ...
        for(int i = 0; i < 5; i+=2)
        {
            if(i == 0)
                CCTV[i].GetComponent<cshCCTVSliderData>().velocity =
                    Mathf.Ceil(speed_coef[i/2] * (CCTV[i].GetComponent<cshCCTVSliderData>().congestion / 100.0f) + speed_const[i/2]);
            else
                CCTV[i].GetComponent<cshCCTVData>().velocity =
                    Mathf.Ceil(speed_coef[i/2] * (CCTV[i].GetComponent<cshCCTVData>().congestion / 100.0f) + speed_const[i/2]);
        }
